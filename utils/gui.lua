...
function Gui.add_close_button(parent, name)
  local button = parent.add { type = 'button', name = name, caption = { 'gui_util.close_button' }, style = 'back_button' }

  Gui.set_style(button, Styles.default_close.style)

  return button
end

function Gui.add_dragger(parent, target)
...
local closable_frame_button_name = Gui.uid_name()
...
function Gui.add_closable_frame(player, params)
  ...
  do -- title
    ...
    local close_button = title_flow.add {
      type = 'sprite-button',
      name = params.close_button or closable_frame_button_name,
      sprite = 'utility/close',
      clicked_sprite = 'utility/close_black',
      style = 'close_button',
      tooltip = 'Close',--{'gui.close-instruction'}
    }
...
Gui.on_click(closable_frame_button_name, function(event)
...
if _DEBUG then
  ...
  local matching_path = '^.+planetary%-wars__/(.+)$'
